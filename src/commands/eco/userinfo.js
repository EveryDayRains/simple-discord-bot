    ...
    User.findOne(
      ...
      async (err, row) => {
        ...
        if (
          ...
          Object.keys(member.presence.clientStatus).length > 0
        ) {
          for (let device in member.presence.clientStatus) {
            statuss += `${statuses[member.presence.clientStatus[device]]} ${
              devices[device]
            }\n`;
          }
        } else {
        ...
        let embed = new Embed()
          ...
          .addField(
            ...
            `${strftime(
              "%d.%m.%Y в %H:%M",
              new Date(message.guild.member(member).joinedTimestamp)
            )}`,
            true
          )
          ...
          .addField(
            "Уровень",
            `${row.level} ур. \`[${row.xp}/${
              parseInt(row.level) + parseInt(1) * 250
            }] XP\``,
            true
          )
