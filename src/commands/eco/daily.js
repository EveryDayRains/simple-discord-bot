...
run(message,args){
    ...
    //this.client.database.query(`UPDATE economy SET money = ${moneyu} + ${randommoney} WHERE uid = ${message.author.id} AND sid = ${message.guild.id}`)
    row.money += randommoney
    row.cd = add
    row.save();
    //this.client.database.query(`UPDATE economy SET cd = ${add} WHERE uid = ${message.author.id} AND sid = ${message.guild.id}`)
