...
run(message,args){
    ...
    if(!args[1])
    return utils.error(message, 'NO_ARGS', { usage: `${this.client.config.guild.get(message.guild.id).prefix + this.options.name} ${this.options.usage}` });
    let rUser = message.guild.member(message.mentions.users.first() ||bot.users.cache.get(args[0]) || message.guild.members.get(args[0]));
    embed.setTitle(`**Ответ разработчика**`)
        .setColor('RANDOM');
