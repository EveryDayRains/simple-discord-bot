...
run(message,args){
    ...
    utils.osu.getUser({ u: args[0] }).then(user =>{
        embed.setAuthor("Информация о пользователе в osu!")
        .addField("Ник", `[${user.name}](https://osu.ppy.sh/users/${user.id})`, true)
        ...
        .addField("Ранг", `#${user.pp.rank}`, true)
        .addField("Ранг в стране", `#${user.pp.countryRank}`, true)
        .addField('Сыграно игр', user.counts.plays, true)
